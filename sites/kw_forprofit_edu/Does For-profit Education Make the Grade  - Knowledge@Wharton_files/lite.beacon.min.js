...
isCSyncEnabled:legacyOpts.isCSyncEnabled,rr:"",informerID:legacyOpts.informer,pubCategoryId:legacyOpts.pubCategoryId,isQuantcastEnabled:legacyOpts.isQuantcastEnabled,isComscoreEnabled:legacyOpts.isComscoreEnabled,dataProviders:legacyOpts.dataProviders,threePTrackers:legacyOpts.threePTrackers,beacon:legacyOpts.beacon};if(legacyOpts.viglink&&legacyOpts.viglink.apiKey!=="0")payload.viglink={vigLinkCUID:legacyOpts.viglink.vigLinkCUID,apiKey:legacyOpts.viglink.apiKey,libURL:legacyOpts.viglink.urlOfLib};
this.fireBeacon(type,payload)},beaconFIRED:false,fireBeacon:function(type,payload){try{payload=payload||this.lastPayload;if(payload.informer_uri){var wijitTrack=new Image;var query="uri="+encodeURIComponent(payload.informer_uri);query+="&informer="+encodeURIComponent(payload.informerID);query+="&viewId="+this.guid();query+="&rand="+(new Date).getTime();if(typeof payload.rr!=="undefined"&&payload.rr.length>0)query+="&rr="+encodeURIComponent(payload.rr);else if(document.referrer.length>0)query+="&rr="+
encodeURIComponent(document.referrer);query+="&type="+type;var wijitTrackUrl=payload.image_tracking+payload.tracker_uri+"?"+query;if(!this.beaconFIRED){wijitTrackUrl+="&beacon=1";this.beaconFIRED=true;this.beaconWijit=type;wijitTrack.onload=function(){try{LJT_Lite.readerBeacon(type,payload)}catch(Err){LJT_Lite.errors.push(["readerBeacon",Err])}};wijitTrack.src=wijitTrackUrl;if(payload.viglink)try{this.viglink(payload.viglink,payload)}catch(Err){this.errors.push(["Viglink",Err])}try{this.thirdParty(payload)}catch(Err){this.errors.push(["ThirdParty",
...
query+="&v=1.0";query+="&type="+type;if(payload.isCSyncEnabled)query+="&csync=1";if(typeof payload.rr!=="undefined"&&payload.rr.length>0)query+="&rr="+encodeURIComponent(payload.rr);else if(document.referrer.length>0)query+="&rr="+encodeURIComponent(document.referrer);var beaconInit=false;var beacon=document.createElement("iframe");if(beacon.style){beacon.style.border="0";beacon.style.width="1px";beacon.style.height="1px";beacon.style.display="none";beaconInit=true}else if(typeof beacon.setAttribute==
"function"){beacon.setAttribute("width","1px");beacon.setAttribute("height","1px");beacon.setAttribute("border","0");beaconInit=true}var url="";if(payload.beacon_url)beacon.src=payload.beacon_url+"?"+query;else beacon.src=payload.image_tracking+"beacon?"+query;if(document.body&&beaconInit)document.body.appendChild(beacon)}}}catch(Err){this.errors.push(Err)}return global}();